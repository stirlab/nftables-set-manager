 ...
 * dns: Gets the IP address(es) of a hostname using the built-in DNS resolver
 * apt_list: Gets IP addresses for all Apt sources files (requires ```python3-apt``` / ```aptsources``` when this plugin is used)
